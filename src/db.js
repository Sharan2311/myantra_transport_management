...
const vehicleFromDB = r => ({
  ...
  pouchExempt: r.pouch_exempt||false, pouchOverride: r.pouch_override!=null?+r.pouch_override:null, shortageOwed: +(r.shortage_owed||0),
...
const vehicleToDB = v => ({
  ...
  pouch_exempt: v.pouchExempt||false, pouch_override: v.pouchOverride!=null&&v.pouchOverride!==""?+v.pouchOverride:null, shortage_owed: v.shortageOwed||0,
...
export const DB = {
  ...
  getDieselRequests: async () => {
    try {
      ...
      return (data||[]).map(r => ({
        ...
        pumpId: r.pump_id,
        amount: +(r.amount||0),
        dieselAmount: r.diesel_amount!=null ? +(r.diesel_amount) : null,
  ...
  saveDieselRequest: async (r) => {
    const { error } = await supabase.from('mye_diesel_requests').upsert({
      ...
      amount: r.amount||0,
      diesel_amount: r.dieselAmount??null,
